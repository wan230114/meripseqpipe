...
args <- c("formatted_designfile.txt", "rank","independence", "QCPeakPlot.RData")
...
for( file in anno.files.list ){
  anno.table <- read.table(file, header=F, sep="\t", quote="", stringsAsFactors = F)[,c(1,2,3,15,11,12,13,14,17)]
  colnames(anno.table) <- c("Chr","ChrStart","ChrEnd","ID","Gene_symbol","Coding","Location","Relative_distance","RNA_type")
  anno.table[anno.table$Location == "CDS" & anno.table$Relative_distance <= 5, 7] <- "Start Codon"
  anno.table[anno.table$Location == "5UTR" & anno.table$Relative_distance >= 95,7] <- "Start Codon"
...
distribute.table$Location <- factor(distribute.table$Location, levels = c("intron", "3UTR", "Stop Codon", "CDS", "Start Codon", "5UTR"))
...
distribute.barplot <- ggplot(distribute.table[distribute.table$Location %in% c("Start Codon", "Stop Codon"),],aes(group, value, fill = Location)) +
                        geom_bar(stat="identity",position = 'fill') + coord_flip() +
                        ggtitle("Peaks Distribution") +
                        scale_y_continuous(expand = c(0, 0)) +
                        guides(fill = guide_legend(reverse = TRUE)) +
                        ...
                        theme(panel.grid =element_blank(), #remove grid line
                              ...
                              axis.title = element_blank(),
                              axis.ticks.x = element_blank()) #remove ticks
distribute.barplot.count <- ggplot(distribute.table[distribute.table$Location %in% c("Start Codon", "Stop Codon"),],aes(group, value, fill = Location)) +
                            geom_bar(stat="identity") + coord_flip() +
                            ggtitle("Peaks Distribution") +
                            scale_y_continuous(expand = c(0, 0)) +
                            guides(fill = guide_legend(reverse = TRUE)) +
...
QC.motif.filelist = dir(".",pattern = "motif[1,2,3,4,5,6,7,8,9]*.motif",recursive = TRUE)
QC.motif.list <- NULL
...
# pdf("all-motif.pdf", width = 5.7, height = 2.7)
pdf("all-motif.pdf", width = 5.7, height = 2.7)
for (x in QC.motif.list){
  for (xx in x){
    print(xx)
  }
}
